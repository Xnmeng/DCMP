...
if __name__ == "__main__":
    ...
    parser.add_argument('--auxiliary_pretrained', default='/home/lc/models/CLIP-ViT-H-14-laion2B-s32B-b79K/open_clip_model.safetensors', type=str, help='For convenience, we use the openclip library to import the auxiliary model ViT-H-CLIP') 
    parser.add_argument('--auxiliary_model_name', default='ViT-H-14', type=str)
    ...
    # AUXILIARY MODEL
    # ----------------------
    auxiliary_model, _, sample_transform = open_clip.create_model_and_transforms(
    args.auxiliary_model_name,
    pretrained=args.auxiliary_pretrained
    )
    ...
    # DATASET & DATALOADER
    # --------------------
    if args.dataset_name in ['imagenet_100', 'imagenet_1k']:
        dataset_root = os.path.join(args.imagenet_root, '')
    else:
        dataset_root = getattr(args, f'{args.dataset_name}_root')
