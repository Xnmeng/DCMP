...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'MaPLe' or current_trainer == 'CoPrompt':
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx=None, compound_deeper_prompts=None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        # After positional embeddings, we will attach prompts with the model, remember only those
        # are trainable parameters here in whole image encoder.
        if self.VPT_shallow and shared_ctx is not None:
            visual_ctx = shared_ctx.expand(x.shape[0], -1, -1).to(dtype=x.dtype)
            x = torch.cat([x, visual_ctx], dim=1)
        elif shared_ctx is None:
            pass
        else:
            assert self.prompt_till_layer_visual == 0
        ...
        if compound_deeper_prompts is None:
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe" or trainer == "CoPrompt":
    ...
    def encode_image(self, image):
        if (self.trainer == "MaPLe" or self.trainer == "CoPrompt") and isinstance(self.visual, VisionTransformer_MaPLe):
    ...
    def encode_text(self, text):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        if self.trainer == "MaPLe" or self.trainer == "CoPrompt":
            outputs = self.transformer([x, [], 0])
            x = outputs[0]
        else:
            x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
