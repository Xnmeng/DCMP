...
def train_one_epoch(args, logger, writer, loader, model, optimizer, epoch, selected_samples_t, selected_samples_i):
    ...
    for batch_idx, (images, labels, img_id, descriptive_text_token, _, mask) in enumerate(tqdm(loader, desc="Training")):
        ...
        loss_lcc_image = feature_consistency_loss(prompted_image_cc, frozen_image_features, criterion=args.lcc_criterion)
        loss_lcc_text = feature_consistency_loss(prompted_text_cc, frozen_text_features, criterion=args.lcc_criterion)
        ...
        loss = (
            ...
            + loss_pseudo_image
            + loss_pseudo_text
            + args.lambda_lcc_image * loss_lcc_image
            + args.lambda_lcc_text * loss_lcc_text
        )
...
def _predict_from_mode(logits_image, logits_text, args):
    ...
    if args.inference_mode in ["image", "csc"]:
        probs = image_probs
    elif args.inference_mode == "text":
        probs = text_probs
    elif args.inference_mode == "soft":
        probs = args.image_vote_weight * image_probs + (1.0 - args.image_vote_weight) * text_probs
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lambda_lcc_image', default=4.0, type=float)
    parser.add_argument('--lambda_lcc_text', default=4.0, type=float)
    parser.add_argument('--lcc_criterion', default='cosine', choices=['cosine', 'mse', 'l1'])

    parser.add_argument('--inference_mode', default='soft', choices=['csc', 'image', 'text', 'soft'],
                        help='csc/image: image branch only; soft: image-text soft voting.')
    parser.add_argument('--image_vote_weight', default=0.5, type=float,
                        help='Used only when inference_mode=soft.')
    ...
    if args.eval_only:
        sweep_weights = [0.3, 0.4, 0.45, 0.5, 0.55, 0.6]

        # 先测 image-only / text-only
        for mode in ['image', 'text']:
            args.inference_mode = mode
            total_acc, old_acc, new_acc = test(model, test_loader_unlabelled, args)
            logger.info(
                f"[Eval] mode={mode} | All {total_acc:.4f} | Old {old_acc:.4f} | New {new_acc:.4f}"
            )

        # 再测 soft voting
        args.inference_mode = 'soft'
        for w in sweep_weights:
            args.image_vote_weight = w
            total_acc, old_acc, new_acc = test(model, test_loader_unlabelled, args)
            logger.info(
                f"[Eval] mode=soft, image_vote_weight={w:.2f} | "
