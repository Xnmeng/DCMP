...
def build_coprompt_prompt_text(text):
    """Build a concise prompt-style text from the full RTG description.

    CoPrompt uses a simpler learnable text input and a richer frozen-text target.
    In OWSSL we approximate that by using the first sentence (base text + top tags)
    as the prompt-side input, while the full RTG description is kept for the frozen
    consistency branch.
    """
    text = text.strip()
    if not text:
...
def build_coprompt_rich_text(text):
    return text.strip()
